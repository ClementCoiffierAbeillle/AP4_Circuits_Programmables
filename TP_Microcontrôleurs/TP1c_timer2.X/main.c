...
void delay_parfait(void) {
    int iterations = 125; // Nombre d'itérations pour obtenir un délai de 125 ms (0.125 secondes)
    for (int i = 0; i < iterations; i++) {
        while (!PIR1bits.TMR2IF) {
            // Attend que l'interruption du Timer 2 se produise
        }
        PIR1bits.TMR2IF = 0; // Réinitialise le drapeau d'interruption du Timer 2
    }
}
...
void init_timer2(void) {
    ...
    PR2 = 124; // Valeur de rechargement pour obtenir un débordement toutes les 1 millisecondes
    T2CONbits.TMR2ON = 1; // Activation du Timer 2
    TMR2 = 0; // Réinitialisation du compteur du Timer 2
}
...
void main(void) {
    ...
    while (1) {
        ...
        // Allume les LEDs une à une avec un délai de 125 ms entre chaque allumage
        LED1 = 1;
        LED8 = 0;
        delay_parfait();
        LED2 = 1;
